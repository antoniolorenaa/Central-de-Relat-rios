...
const badCodeStart = "  // Generate AI Suggestion";
const badCodeEnd = "export function registerReportsRoutes";
const badBlock = code.substring(code.indexOf(badCodeStart), code.indexOf(badCodeEnd) + badCodeEnd.length);

if (code.includes(badCodeStart)) {
  code = code.replace(badBlock, "export function registerReportsRoutes");
}

// Now insert it inside registerReportsRoutes
const goodAiRoute = `
...
if (!code.includes("app.post('/api/academic/reports/:reportId/generate-suggestion'")) {
  code = code.replace("export function registerReportsRoutes(app: express.Express, db: FirebaseFirestore.Firestore, authenticate: any) {", "export function registerReportsRoutes(app: express.Express, db: FirebaseFirestore.Firestore, authenticate: any) {\n" + goodAiRoute);
}
